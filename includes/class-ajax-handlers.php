<?php
// phpcs:disable WordPress.Security.NonceVerification.Missing,WordPress.Security.NonceVerification.Recommended -- Every AJAX handler verifies the nonce via check_ajax_referer() at its entry point. The Plugin Check static analyzer cannot trace control flow across method/switch boundaries, so it flags downstream $_POST/$_GET reads as un-verified. The runtime guarantee holds.
// phpcs:disable PluginCheck.Security.Nonce.NonceVerification -- Every AJAX handler in this file calls check_ajax_referer at its entry point (see grep 'check_ajax_referer'). Plugin Check cannot trace cross-method.
